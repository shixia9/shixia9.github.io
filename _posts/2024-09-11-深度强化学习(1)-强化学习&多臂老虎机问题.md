...
多臂老虎机问题可以表示为一个元组 $$ \langle A, R \rangle $$ ，假设每个时间步长中间拉动一个拉杆，多臂老虎机的目标为最大化一段时间步长 $$ T $$ 内累计的奖励： $$ \max \sum_{t=1}^Tr_t, r_t\sim R(\cdot|a_t) $$ 。其中 $$ a_t $$ 表示在第 $$ t $$ 时间步内拉动某一拉杆的动作， $$ r_t $$ 表示动作 $$ a_t $$ 获得的奖励。其中：
