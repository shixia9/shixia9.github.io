...
多臂老虎机问题可以表示为一个元组 $$ \langle A, R \rangle $$ ：

其中 $$ A $$ 为动作集合，表示一个动作拉动一个拉杆。则动作空间集合为 $$ \{a_1, ..., a_K\} $$ ，其中 $$ a_t \in A $$ 表示任意一个动作；

同时 $$ R $$ 为奖励概率分布，拉动每一个拉杆的动作 $$ a $$ 都对应一个奖励概率分布 $$ R(r, a) $$ ，不同拉杆的奖励分布通常是不同的。
